...
## Part 3 - PR body (real case, from a private backend repo)

The original body was ~700 words plus a bot summary block. The rewrite fits the 150-word budget. What each cut applied:

| Cut | Rule / budget |
|---|---|
| Inline code excerpt of the unscoped queryset | The diff already shows it; the body repeats nothing the reviewer sees one click away. |
| Four-paragraph incident-forensics narrative ("undeterminable", "bounded negative") | Collapsed to one hedged clause: "celery logs show the line was likely never reached". Modality preserved, volume removed. |
| tangential internal-guard aside | Not needed to review this change. Moved to a code comment or ticket when it matters. |
| Test file paths and method names | Visible in the diff; the body states what the tests prove, not where they live. |
| Bot summary block | Bot-owned, regenerable, duplicated its own inline comments. |

Kept, one line each: the defect mechanism, the read-only blast radius (a hedge that must not be upgraded), the fix route through the service layer, the empty-set semantics, and the failing-then-passing test evidence.

Rule of thumb for PR bodies: the description answers "why does this change exist and how do I know it works", the diff answers everything else.
